...
do
    filename=logs/full-ft/TTM'_'Proceed'_'$data'_'$seq_len'_'$pred_len'_'full-ft.log

    echo "[full-ft] data=$data seq=$seq_len pred=$pred_len cdim=$concept_dim bneck=$bottleneck_dim ema=$ema lr=$online_learning_rate"

    python run.py --model TinyTimeMixer --decoder_mode $decoder_mode \
